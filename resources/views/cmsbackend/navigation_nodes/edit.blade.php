...
@section('content')
    <div class="row">
        <div class="col-xs-12">
            <div class="box">
                <div class="box-body">
                    @if($navigation->has_details)
                        @include('cmsbackend.parts.top_nav.module_records', ['active' => 'edit', 'model' => $node])
                        <hr>
                    @endif
                    ...
                    @endif
                    <form role="form" method="POST" action="{{ route('records.edit', [$navigation->id, $node->id]) }}">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="title" name="title" class="form-control" value="{{ old('title') ? : $node->title }}" required />
                                </div>
                            </div>
                            @if($structures = json_decode($navigation->structure))
                                <?php $data = json_decode($node->data); ?>
                                <div class="col-md-12">
                                    <hr>
                                </div>
